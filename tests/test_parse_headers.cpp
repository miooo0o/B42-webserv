...
bool	test_parse_headers(const std::string& str, const std::string& headerStr, int num){
...
int main() {
	...
	/* INVALID HEADERS */
	std::string	headerStr6 = "Host example.com\r\nUser-Agent curl/7.68.0\r\n\r\n";												//Each header must have a colon (:) separating the key and value.
	std::string	headerStr7 = ": value\r\nHost: example.com\r\n\r\n"; 															// A header key cannot be empty.
	std::string	headerStr8 = "X-Header@: value\r\n\r\n"; 																		// Header names must only contain printable ASCII characters except spaces and special symbols like @.
	std::string	headerStr9 = "Header Name : value\r\n\r\n"; 																	// Spaces before the colon are not allowed.
	std::string	headerStr10 = "HeaderName:value\r\n\r\n"; 																		// There should be a space after the colon (:), e.g., "HeaderName: value".
	std::string	headerStr11 = "Content-Length:\r\n\r\n"; 																		// A header value cannot be empty unless it's explicitly allowed by HTTP specifications.
	std::string	headerStr12 = "X-Header: This is a long\r\nheader value\r\n\r\n"; 												// If a header value needs to span multiple lines, the subsequent lines must be indented with at least one space (X-Header: This is a long\r\n value\r\n)
	std::string	headerStr13 = "X-Header: value \x01\x02\x03\r\n\r\n"; 															// Control characters (ASCII < 32, except horizontal tab) are not allowed in header values.
	std::string	headerStr14 = "Content-Length: abc123\r\n\r\n"; 																// Content-Length must be a numeric value.
	std::string	headerStr15 = "Content-Length: -100\r\n\r\n"; 																	// Content-Length cannot be negative.
	std::string	headerStr16 = "Content-Length: 999999999999999999999999999\r\n\r\n";											// An excessively large Content-Length may cause integer overflow issues.
	std::string	headerStr17 = "Accept-Encoding: gzip, deflate,\r\n\r\n"; 														// A comma-separated list must not end with a trailing comma.
	std::string	headerStr18 = "X-Header: Value\r\n Another line\r\n\r\n"; 														// Header continuations require a leading space (X-Header: Value\r\n Another line is valid, but Another line without a space is not).
	std::string	headerStr19 = "Host: example.com\r\nUser-Agent: curl/7.68.0\r\n\r\n\r\n"; 										// Only one CRLF (\r\n\r\n) should separate headers from the body.
	std::string	headerStr20 = "Header:Name: value\r\n\r\n"; 																	// Header names cannot contain colons (:).
	std::string	headerStr21 = "Transfer-Encoding: CHUNKED\r\n\r\n"; 															// HTTP headers are case-insensitive, but values like chunked in Transfer-Encoding are typically lowercase per convention.
	std::string	headerStr22 = " Host: example.com\r\n\r\n"; 																	// Headers cannot start with whitespace.
	std::string headerStr23 = "X-Header: " + std::string(8192, 'A') + "\r\n\r\n";															// Some servers have limits (e.g., 8 KB for Apache) on header line length.
	std::string	headerStr24 = "Set-Cookie: sessionid=abc123\r\nSet-Cookie: userid=xyz456\r\n\r\n"; 								// Some headers (like Set-Cookie) allow duplicates, but others (like Content-Length) must be combined correctly.
	std::string	headerStr25 = "Connection: close\r\nHost: example.com\r\nContent-Length: 10\r\n\r\nGET / HTTP/1.1\r\n\r\n"; 	// Attempting to inject another HTTP request within headers is invalid and a security risk.
	
    assert(test_parse_headers(request, headerStr0, 0));
    assert(test_parse_headers(request, headerStr1, 1));
    assert(test_parse_headers(request, headerStr2, 2));
    assert(test_parse_headers(request, headerStr3, 3));
    assert(test_parse_headers(request, headerStr4, 4));
	assert(test_parse_headers(request, headerStr5, 5));
    assert(test_parse_headers(request, headerStr6, 6));
    assert(test_parse_headers(request, headerStr7, 7));
    assert(test_parse_headers(request, headerStr8, 8));
	assert(test_parse_headers(request, headerStr9, 9));
    assert(test_parse_headers(request, headerStr10, 10));
    assert(test_parse_headers(request, headerStr11, 11));
    assert(test_parse_headers(request, headerStr12, 12));
    assert(test_parse_headers(request, headerStr13, 13));
    assert(test_parse_headers(request, headerStr14, 14));
	assert(test_parse_headers(request, headerStr15, 15));
    assert(test_parse_headers(request, headerStr16, 16));
    assert(test_parse_headers(request, headerStr17, 17));
    assert(test_parse_headers(request, headerStr18, 18));
	assert(test_parse_headers(request, headerStr19, 19));
	assert(test_parse_headers(request, headerStr20, 20));
	assert(test_parse_headers(request, headerStr21, 21));
	assert(test_parse_headers(request, headerStr22, 22));
	assert(test_parse_headers(request, headerStr23, 23));
	assert(test_parse_headers(request, headerStr24, 24));
	assert(test_parse_headers(request, headerStr25, 25));
    return 0;
}

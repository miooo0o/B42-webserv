...
#define URI_PATH_UNSAFE		"400 URI : Query : Unsafe Characters in Path"
#define URI_PORT_INV		"400 URI : Query : Invalid or missing port number in URI"
...
#define URI_TOO_LONG		"400 URI : Path is invalid : "
 */

struct errFlag{
	static const uint64_t NONE = 0;
	static const uint64_t HEADER_END = 1ULL << 1;
	static const uint64_t HEADER_SEPARATOR = 1ULL << 2;
	static const uint64_t METHOD_NOT_PERMITTED = 1ULL << 3;
	static const uint64_t METHOD_UNRECOGNISED = 1ULL << 4;
	static const uint64_t METHOD_UNSUPPORTED = 1ULL << 5;
	static const uint64_t METHOD_CASE = 1ULL << 6;
	static const uint64_t REQUEST_ENDING = 1ULL << 7;
	static const uint64_t URI_ENCODING = 1ULL << 8;
	static const uint64_t URI_FRAG_RECEIVED = 1ULL << 9;
	static const uint64_t URI_ILLEGAL_CHARACTER = 1ULL << 10;
	static const uint64_t URI_PATH_INVALID	= 1ULL << 11;
	static const uint64_t URI_SCHEME_UNSUPPORTED = 1ULL << 12;
	static const uint64_t URI_SCHEME_UNRECOGNISED = 1ULL << 13;
	static const uint64_t VERSION_NONE= 1ULL << 14;
	static const uint64_t VERSION_UNRECOGNISED = 1ULL << 15;
	static const uint64_t VERSION_UNSUPPORTED = 1ULL << 16;
...
class	Error {
	...
	public:			
		void				addErrorFlag(uint64_t error);
							/* CONSTRUCTORS */
							Error( void );
			...
			void			flagStringsInit();
							...
							/* SETTERS */
			void			setError(const std::string& str1, const std::string& str2, int num, errFlag flag);
